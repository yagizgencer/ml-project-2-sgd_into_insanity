...
niiMASK = nib.load('mask.nii.gz')
niiMASK_hdr = niiMASK.get_header()
...
for filename in SUBMISSIONs:
        ...
        for y in range(0,ny):
            for x in range(0,nx):
                ...
                for i in range(0,M) :
                    ...
                    frac_err[i] = abs(frac_true[r] - frac_est[c])
                    AA[r[0],:] = float('Inf')
                    AA[:,c[0]] = float('Inf')
                ...
                if M_true < M_est :
                    if M_true > 0:
                        for i in notUsed_est :
                            ...
                            frac_err = np.append(frac_err, frac_est[i])
                    else :
                ...
                # account for UNDER-ESTIMATES
                    ...
                    if M_est > 0:
                        for i in notUsed_true :
                            err = np.append( err, min( A[i,:] ) )
                            frac_err = np.append(frac_err, frac_true[i])
                    else :
                        err = np.append( err, 45 )
                        frac_err = np.append(frac_err, 1)

                AE[x,y,z] = err.mean()
                FE[x,y,z] = frac_err.sum()
    ...
    values = Pd[niiMASK_idx] #Consider only the voxels indicated by the mask
    # PERCENTAGE ERROR IN THE ESTIMATION OF NUMBER OF PEAKS
    ...
    values = FE[niiMASK_idx]
    # SUM OF FRACTION ERRORS
    XLS_sheet.write(XLS_row, 29, np.mean(values))
    XLS_sheet.write(XLS_row, 30, np.std(values))
    XLS_sheet.write(XLS_row, 31, np.min(values))
    XLS_sheet.write(XLS_row, 32, stats.scoreatpercentile(values, 25))
    XLS_sheet.write(XLS_row, 33, np.median(values))
    XLS_sheet.write(XLS_row, 34, stats.scoreatpercentile(values, 75))
    XLS_sheet.write(XLS_row, 35, np.max(values))
